...
void ControlCenterWindow::applyFonts() const noexcept
{
    ...
    for (const auto& descriptor : sliderDescriptors())
    {
        ...
        setControlFont(slider.label, normalFont_);
        setControlFont(slider.trackbar, normalFont_);
        setControlFont(slider.valueText, normalFont_);
    }
    for (const auto& descriptor : pageControlDescriptors())
    {
        setControlFont(this->*descriptor.control, descriptor.heading ? sectionFont_ : normalFont_);
    }
...
void ControlCenterWindow::updatePageVisibility() noexcept
{
    ...
    for (const auto& descriptor : pageControlDescriptors())
    {
        setPageControlVisible(this->*descriptor.control,
            pageControlVisible(descriptor.page, descriptor.section));
    }
    for (const auto& descriptor : sliderDescriptors())
    {
        const auto& slider = this->*descriptor.control;
        const bool visible = pageControlVisible(descriptor.page, descriptor.section);
        setPageControlVisible(slider.label, visible);
        setPageControlVisible(slider.trackbar, visible);
        setPageControlVisible(slider.valueText, visible);
    }
