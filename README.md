...
即使剩余不足 30 天仍可使用；证书实际过期、缺失、损坏、签名不匹配或证书存储不一致时，仅无边框 WGC 请求会被拒绝并
回退到 FX-only。Host 本身仍可启动和运行其他捕获路径。重新运行当前安装器一次即可重新签名修复。若没有管理员权限，请改用
上面的便携 ZIP，直接解压运行即可，但便携版没有 Package Identity，无法承诺无边框 WGC。
