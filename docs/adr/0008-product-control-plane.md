...
9. Control Center 提供 `input.samplingRateHz` 滑块。Host 先将一次呈现更新内相邻的 Raw Input Move
   收敛为最后位置，Down/Up/Cancel 划分样本组且不被合并；`0` 表示不在这层队列收敛之外额外限频，
   整数 `1..1000` 表示后续 Move 采样率上限。限频用保留样本的消息分派 QPC 判断间隔，发生在
   轨迹/碎片模拟之前，且独立于暂停后的模拟时间和 Unity 空间阈值。切换采样率会清空旧相位，使下一 Move
...
- 采样率为 `0` 时保留每个连续 Move 组的最终位置；限频时只连接被接受的 Move，且空间距离不足的已接受
  样本仍推进时间相位。动态修改采样率后的下一 Move 立即接受。
