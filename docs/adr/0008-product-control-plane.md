...
1. 配置由 `bafx_config` 持有，使用版本化 JSON（当前 schema 为 5）。读取时先完成迁移和
...
5. 首个垂直切片只承诺 `GetState`、`GetConfig`、`SetConfig <schema-5-json>`、
...
- schema 1/2/3/4 配置可迁移到 schema 5；schema 1/2/3 以及 schema 4 缺失字段时默认允许系统捕获
