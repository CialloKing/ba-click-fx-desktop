...
- 默认启用输入健康、路由、前台与覆盖窗口诊断，关联模拟采样、调度原因和逐显示会话的 Present；输入活动按秒汇总，窗口最多每 250 ms 观察，空闲保留 10 秒心跳。
- 保留 Raw Input 读取失败阶段、错误码及取消来源；显示会话使用独立实例编号，避免窗口句柄复用导致计数增量错误，正常退出刷新最后一条诊断。
- 补充运行时日志判读和 Issue #2 复现步骤；Issue #1、#2 仍需用户故障现场数据确认，不将诊断增强表述为问题已修复。
- 控制中心完整状态读取移至后台并校验异步结果版本，保留诊断详情阅读位置。
- 控制中心仅回填变化的控件，显示器与预设列表按内容更新，减少重复重绘并保留正在编辑的颜色和滑块。
- 安装、用户注册和卸载共享状态序列化、摘要、字节配对和账本解析；打包与跨版本恢复同步携带公共模块，并校验历史状态摘要兼容性。
- 主显示器输出协商、色彩刷新和有限重试移入独立运行模块；主循环保留安全执行时机，WGC 停止顺序和失败关闭行为保持一致。
- 特效字段注册表接管普通字段的必填读取，保留原有错误顺序、类型约束、主题色归一化和拖尾寿命联动。

## 0.2.18 - 2026-09-17

- 显示页状态查询与解析移到后台，合并重复请求、丢弃失效结果，减少界面线程等待；完整刷新保留配置一致性校验。
- 集中管理 Host 的 WGC 重试身份与待处理状态，主副显示器共用有限输出重试预算并保留溢出保护。
- 特效配置使用统一字段注册表驱动允许字段、补丁赋值和 JSON 输出，保留参数联动及历史别名边界。
- 32 个滑块与 86 个页面控件使用统一描述表，复用创建、数值回填、字体和页面显隐逻辑。
- 副显示器捕获、恢复和渲染移入独立模块，主副显示器复用同一视觉配置映射。
- Host 控制面与桌面运行时的公共实现改为共享构建目标，应用和对应测试复用同一源码清单及依赖。
- 文档检查覆盖当前路线图并自动发现历史快照，检查其中的本地链接与标题锚点。
- 拆出 Host 显示运行时快照组装与输出协商诊断，保持渲染线程状态所有权、日志字段及 IPC 发布顺序。
- 按职责拆分控制中心布局、显示状态页面，以及 Host 启动选项和性能样本转换；应用与界面测试共用同一
  控制中心实现库，减少重复源码清单与编译配置维护。现有功能、配置和渲染行为保持不变。
- Windows CI 新增现有自动测试、WARP 与文档检查，保留独立的三档 SDK 编译检查，并归档测试报告。
- 精简当前路线图并保留完整历史快照，补充中英文开发指南的模块入口与验证方式。
- 主配置保持 schema 20，IPC 命令和特效预设格式保持兼容，现有配置无需迁移。

## 0.2.17 - 2026-09-16

- 系统页新增“打开日志目录”，可直接在资源管理器中查看日志；清理操作汇总 Host 和控制中心日志结果，
  Host 未连接时仍可清理控制中心日志。
- 共用日志写入器记录写入、轮转、锁等待和维护失败及恢复；跨进程协调追加、轮转和清理，限制单条记录大小，
  降低遗留备份清理频率，避免轮转失败时日志无限增长。
- 控制中心记录启动/退出、连接、操作结果和 IPC 耗时，合并重复轮询错误；Host 记录配置变更来源、前后值、
  代次和错误，超大差异只省略超限内容，保留操作结果。
- Host 补齐退出原因、退出码、运行时长和清理前事件；性能窗口保留最慢帧的同帧上下文，
  并每十秒采样进程内存、句柄和日志阶段耗时，空闲窗口仍可观察资源趋势。
- 主配置保持 schema 20，支持日志保持 schema 2，现有配置与特效预设无需迁移。

## 0.2.15 - 2026-09-14

- 使用 Visual Studio 2026 与 Windows SDK `10.0.28000.0`（补丁版 `28000.2526`）重新构建发行包。
  Full 与 Slim 的 C++/WinRT 编译探测、Release 构建及现有测试通过。
- 录屏兼容模式继续面向 Windows 11 26H2 及以后，保留实际 WGC 能力检查与回退。
  本次更新不改变捕获或渲染逻辑，26H2／OBS 实机录制组合仍待验收。

## 0.2.14 - 2026-09-14

- 录屏兼容测试模式改为 Windows 11 26H2 及以后，最低 OS build 从 `28000` 调整为 `26300`。
  同步中英文界面提示、Host 拒绝信息与使用文档；继续按实际 WGC Session 能力探测和回退，
  版本门槛放行不代表外部录屏兼容性已通过验收。

## 0.2.13 - 2026-09-13

### 新增

- Control Center 支持简体中文与英文。“系统 → 系统行为 → 界面语言”可选择跟随系统、简体中文或 English，
  立即生效，无需重启；Host 未启动、断开或版本不兼容时仍可切换。
- 默认跟随 Windows 当前用户的显示语言：中文使用简体中文，其余语言使用英文。五个页面、高级子页面、
  托盘菜单、应用确认弹窗、动态状态和已知错误说明均支持双语，原始诊断信息和用户自定义名称保持原值。

### 保存与兼容性

- 语言偏好独立保存到 `BAFX.ControlCenter.language`：便携版位于程序目录，安装版位于 `data` 目录。
  重启后保留选择，“重置默认”保留语言偏好；保存失败保持原选择，不覆盖已有偏好。
- 切换语言保留当前页面、显示器、预设、输入内容和快捷键草稿；不发送 Host 配置写入请求。
  内置预设翻译仅影响显示，内部名称、主配置 schema 20、IPC 协议和特效预设格式保持不变。

### 界面与文档

- 系统页将日志清理移入“版本与维护”；页签均分可用宽度，调整英文标签、预设按钮与状态区，
  保留 `860 × 600` 最小逻辑客户区。
- 更新中英文 README 的语言入口和偏好文件说明。发行包不携带本机语言偏好。

## 0.2.12 - 2026-09-10

### 修复

- 证书和 Package 信任校验限定在无边框 WGC 请求路径。证书过期、缺失或签名不匹配时，Host 仍可启动，
  只有无边框 WGC 回退到 FX-only。
- Control Center 不再因为目标机证书过期而阻止安装版 Host 启动；安装状态页会提示无边框 WGC 的回退结果。

### 发布验证

- Full `release-verify` 通过 `45/45`，Slim `slim-release-verify` 通过 `44/44`。
- Full 安装器候选完成未签名 Sparse 模板、原生签名器、Inno Setup 和 PE 静态依赖检查。
- Windows 11 目标硬件上的无边框授权、DWM 最终像素和真实用户授权仍未验收，本版本不新增相关硬件声明。
...
- 同步 Unity 重建工程更新后的材质、Shader、审计文档和基线图清单；原生渲染已符合更新后的材质队列、
  背面剔除和深度语义，不需要改变运行时绘制逻辑。
...
- 最终 Portable 候选的 144 DPI 整窗捕获确认仓库按钮与 Star 提示完整、启用且没有重叠。Full 便携
  ZIP、ZIP 哈希、安装器和安装器哈希四个候选资产已生成并通过本地校验。
