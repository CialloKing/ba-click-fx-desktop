...
- GitHub Actions records the repository Star count daily at 03:17 Asia/Shanghai (scheduling may be delayed).
- Bootstrap rows marked \`reconstructed\` are derived from current stargazers and cannot recover removed Stars.
- Rows marked \`observed\` contain the API count and its actual observation time; counts may decrease.
- Missed observation dates remain absent; no snapshots are interpolated.
- Both the README chart and CSV link refer to this repository only.
...
export function renderStarHistorySvg(rows, repository)
{
  ...
  const lines = [
    ...
    '  <rect x="0.5" y="0.5" width="719" height="359" rx="8" fill="#ffffff" stroke="#d0d7de"/>',
    `  <text x="${margin.left}" y="38" fill="#24292f" font-family="-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" font-size="22" font-weight="600">${title}</text>`,
...
export async function updateStarHistory(options)
{
  ...
  // Bootstrap reconstruction cannot recover removed Stars; record today's actual total too.
...
async function main()
{
  ...
  if (args.help)
  {
    console.log('Usage: node tools/update-star-history.mjs --data-dir <path> [--bootstrap]');
    return;
  }
