...
`NotAfter` 尚未到达时，即使剩余不足 30 天仍可使用；证书实际过期、缺失、损坏、签名不匹配或证书存储不一致时，Host
会 fail-closed，重新运行当前安装器一次即可重新签名修复。卸载可从开始菜单或 Windows“已安装的应用”执行，默认保留安装目录
...
剩余不足 30 天的证书不会触发提示；证书已过期时会阻止安装版 Host 启动，请重新运行当前安装器重新签名修复。
