...
- `BAFX.ControlCenter.exe` 的原生 Win32 控制面：基础页管理启用状态、点击特效、鼠标拖尾、拖尾常驻、
  ...
  以及“打开日志目录”和“清理诊断日志”按钮；确认清理后汇总控制中心与 Host 的删除文件数、释放字节数和
  失败文件数，Host 未连接时仍可清理控制中心日志。“版本与维护”区域显示
  ...
  “重置默认”经确认后恢复其他持久化设置，但保留 Host 已保存的快捷键、当前暂停或运行状态和语言偏好。
- 控制中心支持简体中文与英文，在“系统 → 系统行为 → 界面语言”即时切换，Host 未启动时也可操作。
  默认跟随 Windows 当前用户显示语言；语言偏好独立保存在 `BAFX.ControlCenter.language`，便携版位于程序
  目录，安装版位于 `data` 目录。切换保留页面、显示器、预设和未保存草稿，不发送 Host 配置写入请求。
...
- Spout2 发送器名称固定为 `ba-click-fx-desktop`，输出合同为
  `BGRA8 + sRGB encoding + extended premultiplied alpha + FX-only v6`。空闲帧严格透明；圆盘保留
  Cross2 coverage Alpha，纯加法圆环、碎片、拖尾和 Bloom 只使用一个 BGRA8 Alpha 步进
  防止接收端清除，仍允许 `RGB > Alpha`。RGB 在发送端使用 sRGB 编码，恢复 0.2.2 的圆盘、
  Bloom、圆环、碎片和拖尾可见度；输出不混入或依赖 WGC 桌面背景；
...
- `GetState.productVersion` 使用规范 `MAJOR.MINOR.PATCH` 标识 Host 版本。只有 Host 与 Control Center
  ...
  启动和关闭入口继续可用。当前主配置使用 schema 20；该 schema 由 0.2.10 引入全局快捷键配置。
...
签名器。公钥只导入 `LocalMachine\TrustedPeople`，签名后使用 `-DeleteKey` 删除安装器新建证书的私钥。每次运行当前版本
安装器（包括同版本修复）都会生成新的目标机证书并重新签署新的 `.msix`，不会复用旧包或旧证书。证书只按实际有效期判断：
`NotAfter` 尚未到达时，即使剩余不足 30 天仍可使用；证书实际过期、缺失、损坏、签名不匹配或证书存储不一致时，仅无边框 WGC
请求会 fail-closed 并回退 FX-only，Host 仍可启动。重新运行当前安装器一次即可重新签名修复。卸载可从开始菜单或 Windows“已安装的应用”执行，默认保留安装目录
...
若安装器仍报告恢复未完成，请保留安装目录中的状态、暂存和回滚文件，并提供安装日志；不要手动删除它们或证书。有效但
剩余不足 30 天的证书不会触发提示；证书已过期时安装版 Host 仍可启动，但无边框 WGC 会回退 FX-only，请重新运行当前安装器重新签名修复。
